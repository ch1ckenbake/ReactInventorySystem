...
// Production-ready minimal backend (CJS) — core offline features only (no Google Drive sync)
...
function logHistory(action, entityType, entityId, entityName, details, oldValues, newValues) {
  try {
    const id = randomUUID();
    db.prepare(`INSERT INTO history (id, action, entity_type, entity_id, entity_name, details, old_values, new_values) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, action, entityType, entityId, entityName, details || null, oldValues ? JSON.stringify(oldValues) : null, newValues ? JSON.stringify(newValues) : null);
  } catch (err) {
...
app.post('/inventory', (req, res) => {
  ...
  const id = randomUUID();
  try {
    db.prepare(`INSERT INTO inventory (id, batch_code, category_id, variety_id, packaging_id, quantity_packages, total_weight, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, batchCode, categoryId, varietyId, packagingId, quantityPackages, totalWeight, status, createdAt || new Date().toISOString());
...
app.put('/inventory/:id', (req, res) => {
  ...
  try {
    ...
    const result = db.prepare(`UPDATE inventory SET batch_code = ?, category_id = ?, variety_id = ?, packaging_id = ?, quantity_packages = ?, total_weight = ?, status = ?, created_at = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(batchCode, categoryId, varietyId, packagingId, quantityPackages, totalWeight, status, createdAt || oldInventory.created_at, id);
